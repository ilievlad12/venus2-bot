...
@app.route('/')
def home():
    return "Venus2 Bot: Sistemul Ultra Design este Online!"
...
def create_hp_bar(current, maximum):
    ...
    # Alegem culoarea în funcție de procentaj
    if percentage > 0.6:
        color_emoji = "🟩" # Sănătos
    elif percentage > 0.3:
        color_emoji = "🟨" # Lovit
    else:
        color_emoji = "🟥" # Aproape mort
    ...
    async def ataca_button(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        if user not in self.participants:
            if len(self.participants) >= 2:
                return await interaction.response.send_message("❌ Lupta este deja ocupată de 2 eroi!", ephemeral=True)
        ...
        if self.current_hp > 0:
            embed = discord.Embed(
                ...
                description=(
                    ...
                    f"━━━━━━━━━━━━━━━━━━\n"
                    f" dernière lovitură: {user.mention} (`-50 HP`)"
                ),
                color=0x2ECC71
            )
        ...
        # BOSS MORT
            ...
            embed_dead.add_field(
                name="📦 Informații Drop", 
                value="🪙 **50 DC** - Standard\n💎 **150 DC** - Tag [Venus2]", 
                inline=False
            )
...
async def boss(ctx):
    ...
    embed = discord.Embed(
        ...
        description=(
            f"Un inamic puternic a apărut! Se pot înscrie **2 eroi**.\n\n"
